...
# for i, m in enumerate(measures):
    ...
    with h5py.File(f"/Volumes/Results/dataloc/pv50-nu4-urlx.c0sat{s}.T170/"
        ...
        if i == len(ss) - 1:
            ...
            ax.set_xticks(np.linspace(times[s][0], times[s][1], 7, dtype=int))
            ax.set_xticklabels(np.linspace(0, 300, 7, dtype=int))
        else:
            ax.set_xticklabels([])

fig.suptitle(r"(f)")
fig.subplots_adjust(left=0.17, right=0.95, top=0.94, bottom=0.075, hspace=0, wspace=0)
