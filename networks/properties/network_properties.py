...
def calc_centrality(cm, lon, lat, times, savename, dpi=200, area_weighted=False):
    ...
    if area_weighted is False:
        ...
        out_centrality = np.sum(cm, 0) * np.cos(lat) / np.sum(np.cos(lat))
        in_centrality = np.sum(cm, 1)*np.cos(lat) / np.sum(np.cos(lat))
...
def calc_cum_prob_distrib(matrix, measure, savename, dpi=200):
