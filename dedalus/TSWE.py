...
Dedalus script simulating the thermal shallow water equations on a sphere.
Data is saved to HDF5 files in the folder data/model which must exist.
...
stop_sim_time = 10 * hour

# Spectral parameters:
nmodes = 128  # number of spectral modes (powers of two are computationally efficient, especially for FFT algorithms)
...
T['g'] = np.vstack([-np.cos(phi)**2]*Ntheta)
...
#%% Problem
problem = d3.IVP([u, h, T], namespace=locals())
problem.add_equation("dt(h) + nu_hp*lap(lap(h)) + H*div(u) = - div(h*u)")
problem.add_equation("dt(T) + nu_hp*lap(lap(T)) = - u@grad(T)")
problem.add_equation(
    "dt(u) + nu_hp*lap(lap(u)) + g*grad(h) + 2*Omega*zcross(u) + (H/2)*grad(T) = - u@grad(u) - grad(T*h) + (h/2)*grad(T)")
...
#%% Analysis
snapshots = solver.evaluator.add_file_handler('data/model/TSWE_snapshots', iter=int(10 * 60 * second / timestep),
